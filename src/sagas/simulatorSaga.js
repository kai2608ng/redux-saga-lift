...
function* processPassengerOutside({ startingFloor }) {
	...
	if (enterLiftResult.failure) {
		yield put(liftActions.pendingLift({ pendingFloor: startingFloor }));
...
function* processPassengerInside({ destinationFloor }) {
	...
	if (exitLiftResult.failure) {
		yield put(liftActions.pendingLift({ pendingFloor: destinationFloor }));
...
function* pollPassengers() {
	yield fork(function* () {
		while (true) {
			/* A passenger will request a lift every 0 to 20 seconds. */
